...
public class Containerhelper {
    ...
    public void setStartSetting(String title, String hakbeon, JSONObject mysubject, JSONObject majorinfo, JSONObject gradeinfo, JSONObject db, addcontainer container){
        this.title=title;
    ...
    public void cultureContainerCreate(){
        try {
            ...
            subMaxCredit=gradePointDetail;

            //여기부터는 첫번째 컨테이너만 적용되는 리소스를 쓸 차례.
...
//                JSONObject db=getCultureDB(title,hakbeon);
...
//                if(intoarr==null){
//                    JSONObject otherdb=getCultureDB(title,curri_year);
//                    Iterator otherdbI=otherdb.keys();
//                    while(otherdbI.hasNext()){
//                        String imsi2=otherdbI.next().toString(); //imsi2는 db내의 일본어1:외국어 이렇게 쭉쭉 내려가는걸 저장하게 된다
//                        if(subject_nm.equals(imsi2)){
//                            intoarr=subject_nm;
//                            intoarr2=otherdb.get(subject_nm).toString();
//                            break;
//                        }
//                    }
//                }
